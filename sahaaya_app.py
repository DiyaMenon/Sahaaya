...
def display_emergency_steps(emergency: Dict):
    ...
    if 0 <= st.session_state.current_step < len(steps):
        ...
        display_single_step(current_step, st.session_state.current_step + 1, is_current=True)
    
    # Show all steps option
    if st.checkbox("📋 Show all steps overview", key="show_all"):
        st.markdown("### Complete Instructions Overview:")
        for i, step in enumerate(steps):
            display_single_step(step, i + 1, is_current=(i == st.session_state.current_step))

def display_single_step(step: Dict, step_number: int, is_current: bool = False):
    """Display individual step card"""
    ...
    # Voice controls for current step
    if is_current:
        col1, col2 = st.columns([1, 3])
        with col1:
            if st.button("🔊 Listen", key=f"speak_step_{step_number}"):
...
def create_map_features():
    """Create functional map and location features"""
    st.markdown("### 🗺️ Find Nearby Help")
    
    if st.session_state.location_helper:
        col1, col2 = st.columns(2)
        ...
        with col1:
            if st.button("🏥 Find Hospitals", key="find_hospitals"):
                with st.spinner("📍 Finding nearby hospitals..."):
                    try:
                        location = st.session_state.location_helper.get_current_location()
                        if location:
                            hospitals = st.session_state.location_helper.find_nearby_hospitals(*location)
                            ...
                            if hospitals:
                                ...
                                for hospital in hospitals[:3]:  # Show top 3
                                    st.markdown(f"""
                                    <div class="glass-card">
                                        <h4 style="margin: 0 0 10px 0;">🏥 {hospital['name']}</h4>
                                        <p style="margin: 5px 0;">📍 {hospital['address']}</p>
                                        <p style="margin: 5px 0;">📞 {hospital['phone']}</p>
                                        <p style="margin: 5px 0;">📏 Distance: {hospital['distance_km']} km</p>
                                        {"<span style='color: #4facfe;'>⚡ Emergency Services Available</span>" if hospital['emergency'] else ""}
                                    </div>
                                    """, unsafe_allow_html=True)
                            else:
                                st.info("No hospitals found in demo mode")
                        else:
                            st.error("❌ Could not determine location")
                    except Exception as e:
                        st.error(f"❌ Location service error: {e}")
        
        with col2:
            if st.button("🗺️ Open Maps", key="open_maps"):
                try:
                    location = st.session_state.location_helper.get_current_location()
                    if location:
                        maps_url = st.session_state.location_helper.generate_maps_url(*location, "hospital")
                        st.markdown(f'<a href="{maps_url}" target="_blank">🔗 Click here to open Google Maps</a>', unsafe_allow_html=True)
                        st.success("✅ Maps link generated")
                    else:
                        # Fallback generic maps link
                        generic_maps = "https://www.google.com/maps/search/hospital+near+me"
                        st.markdown(f'<a href="{generic_maps}" target="_blank">🔗 Find hospitals near me</a>', unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"❌ Maps error: {e}")
    else:
        st.info("🗺️ Location services not available")
